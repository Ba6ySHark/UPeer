...
const Helpers = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [allPosts, allCourses] = await Promise.all([
          postService.getPosts(selectedCourse, 'offering'),
          courseService.getAllCourses()
        ]);
        
        console.log('Received helper posts details:', allPosts);
        // Enhance posts with has_group property
  ...
  const handleCreatePost = async (postData) => {
    try {
      const newPost = await postService.createPost(postData.content, postData.course_id, 'offering');
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Helpers Board</h1>
        <p className="text-gray-600">
          Browse posts from people who are offering help with courses.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-white rounded-lg shadow">
          <h3 className="mt-2 text-lg font-medium text-gray-900">No help offers yet</h3>
          <p className="mt-1 text-sm text-gray-500">
            Be the first to offer help with your courses!
          </p>
          <div className="mt-6">
            <button
